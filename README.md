...
| 用户名 | 密码 | 角色 | 能做什么 |
|--------|------|------|---------|
| 张三 | 123456 | 顾客 | 浏览菜品、下单、管理地址 |
| 李四 | 123456 | 顾客 | 同上 |
| 老王餐厅 | 123456 | 商家 | 管理菜品、查看订单、指派配送 |
| 川味轩 | 123456 | 商家 | 同上 |
| 赵六 | 123456 | 配送员 | 查看配送任务、确认送达 |

---

## 常见问题

**Q: `mysql` 命令提示"不是内部或外部命令"？**

A: MySQL 没加入系统 PATH。找到 MySQL 安装目录（一般是 `C:\Program Files\MySQL\MySQL Server 8.0\bin`），把这个路径加到系统环境变量的 PATH 里，然后重新打开 CMD。

**Q: `npm install` 特别慢或者报网络错误？**
...
A: 密码不对。打开 `backend/cfg.py` 确认 `DB_PASSWORD` 和你的 MySQL 密码一致。

**Q: 前端页面打开是空白 / 报 404？**

A: 确认后端在运行（那个 CMD 窗口没关），然后刷新页面。
